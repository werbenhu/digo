...
// init_ registers the singleton object with ID  into the DI object manager
// Now you can retrieve the singleton object by using `obj, err := di.Provide("")`.
// The obj obtained from the above code is of type `any`.
// You will need to forcefully cast the obj to its corresponding actual object type.
func init_() {
	...
	_obj := NewUserController(name)
	digo.RegisterSingleton("", _obj)
}
...
func group_group_controllers_NewUserController() {
	...
	digo.RegisterSingleton("group.controllers", member)
}
