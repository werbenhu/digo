...
func (g *Generator) defineProviderFunc(fn *DiFunc) *ast.FuncDecl {
	...
	stmts = append(stmts, &ast.AssignStmt{
		...
		X: newCallExpr(newSelectorExpr(g.RegisterFunction), newExprs(
			newBasicLit(fn.provider),
			newIdent(fn.providerObjName())),
		),
	})

	comments := []string{
		fmt.Sprintf("\n// %s registers the singleton object with ID %s into the DI object manager", fn.providerFuncName(), fn.provider),
		fmt.Sprintf("// Now you can retrieve the singleton object by using `obj, err := di.Provide(\"%s\")`.", fn.provider),
...
func (g *Generator) defineGroupFunc(fn *DiFunc) *ast.FuncDecl {
			...
			&ast.AssignStmt{
				...
				Rhs: newExprs(
					newCallExpr(
						...
						[]ast.Expr{newBasicLit(fn.provider)},
					),
				),
			},
			newErrCheckStmt(),
	...
	stmts = append(stmts, &ast.ExprStmt{
		X: newCallExpr(newSelectorExpr(g.RegisterFunction), newExprs(
			newBasicLit(fn.group),
			newIdent("member")),
		),
	})

	comments := []string{
		fmt.Sprintf("\n// Add a member object to group: %s", fn.group),
		fmt.Sprintf("// Now you can retrieve the group's member objects by using `objs, err := di.Members(\"%s\")`.", fn.group),
...
func (g *Generator) output() error {
	if err := os.MkdirAll(g.pkg.folder, 0777); err != nil {
		return err
	}

	path := filepath.Join(g.pkg.folder, g.GeneratedFileName)
